...
class KnowledgeRepository:
    ...
    def log_event(self, kind: str, detail: str = "", unit_id: str = "",
        ...
        with self._conn:
            self._conn.execute(
                "INSERT INTO events(at, kind, unit_id, raaga, source_id, detail)"
                " VALUES (?,?,?,?,?,?)",
    ...
    def has_source(self, provider: str, locator: str) -> bool:
        ...
        return self._conn.execute(
            "SELECT 1 FROM sources WHERE fingerprint=?", (fp,)).fetchone() is not None
    ...
    def reject_phrase(self, phrase_id: str, reason: str = "") -> None:
        with self._conn:
            self._conn.execute(
                "UPDATE phrases SET rejected=1, confidence=0.0, notes=? WHERE id=?",
                (reason, phrase_id))
            ...
            if row["value"] == fact.value:
                with self._conn:
                    self._conn.execute(
                        ...
                        (min(0.99, max(row["confidence"], fact.confidence) + 0.05),
                         row["id"]))
                return
        # An "observed_*" entry is evidence from one source, not a claim about
        # the raaga: two recordings showing different notes do not contradict
        # each other. Only canonical claims can be in dispute.
    ...
    def save_progress(self, progress: UnitProgress) -> None:
        with self._conn:
            self._conn.execute(
                "INSERT INTO curriculum_progress(unit_id, raaga, status, mastery,"
                " attempts, failures, last_attempted_at, completed_at, notes)"
                " VALUES (?,?,?,?,?,?,?,?,?)"
        ...
        with self._conn:
            self._conn.execute(
                ...
                (feedback_id, time.time(), target_kind, target_id, raaga, text,
                 sentiment))
        self.log_event("feedback.received", f"{sentiment}: {text[:80]}", raaga=raaga)
        return feedback_id
